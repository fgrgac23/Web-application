...
export class SadrzajDAO {
    ...
    async dajSadrzajPoTmdbId(tmdbId) {
  ...
  `;
    ...
    async dodajSadrzajUKolekciju(sadrzaj_id, kolekcija_id) {
    ...
    async promjeniVidljivost(sadrzaj_id, javno) {
        let sql = `UPDATE sadrzaj SET javno = ? WHERE sadrzaj_id = ?`;
        await this.baza.ubaciAzurirajPodatke(sql, [javno, sadrzaj_id]);
    }
    async obrisiSadrzajIzKolekcije(kolekcija_id, sadrzaj_id) {
        const sql = "DELETE FROM kolekcija_sadrzaj WHERE kolekcija_id = ? AND sadrzaj_id = ?";
        await this.baza.ubaciAzurirajPodatke(sql, [kolekcija_id, sadrzaj_id]);
    }
    async sadrzajJeUKojimKolekcijama(sadrzaj_id) {
        const sql = "SELECT COUNT(*) AS broj FROM kolekcija_sadrzaj WHERE sadrzaj_id = ?";
        const rezultat = await this.baza.dajPodatke(sql, [sadrzaj_id]);
        if (!Array.isArray(rezultat) || rezultat.length === 0) {
            return false;
        }
        const prviRed = rezultat[0];
        if (typeof prviRed.broj !== "number") {
            return false;
        }
        return prviRed.broj > 0;
    }
    async obrisiSadrzaj(sadrzaj_id) {
        const sql = "DELETE FROM sadrzaj WHERE sadrzaj_id = ?";
        await this.baza.ubaciAzurirajPodatke(sql, [sadrzaj_id]);
    }
}
//# sourceMappingURL=sadrzajDAO.js.map