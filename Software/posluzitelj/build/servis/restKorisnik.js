...
export class RestKorisnik {
    ...
    async postKorisnici(zahtjev, odgovor) {
        ...
        const token = kreirajToken({ korime: noviKorisnik.korime }, this.konf.dajKonf().jwtAktivacija);
        const link = `http://localhost:12222/api/korisnik/aktivacija/${token}`;
        await posaljiMail(noviKorisnik.email, "Aktivacija korisničkog računa", `Aktivirajte račun: ${link}`, this.konf);
        odgovor.json({
    ...
    async aktivacijaRacuna(zahtjev, odgovor) {
        ...
        try {
            const podaci = jwt.verify(tokenParam, this.konf.dajKonf().jwtAktivacija);
