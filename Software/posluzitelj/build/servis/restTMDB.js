...
export class RestTMDB {
    ...
    async getSlike(zahtjev, odgovor) {
        let sesija = zahtjev.session;
        if (sesija.korisnik == null) {
            odgovor.status(401);
            odgovor.send({ greska: "Neautoriziran pristup!" });
    ...
    async getVideo(zahtjev, odgovor) {
        let sesija = zahtjev.session;
        if (sesija.korisnik == null) {
    ...
    async dodajSadrzaj(zahtjev, odgovor) {
        let sesija = zahtjev.session;
        if (sesija.korisnik == null) {
            odgovor.status(401);
            odgovor.send({ greska: "Neautoriziran pristup!" });
    ...
    async dodajSadrzajUKolekciju(zahtjev, odgovor) {
        let sesija = zahtjev.session;
        if (sesija.korisnik == null) {
            odgovor.status(401);
            odgovor.send({ greska: "Neautoriziran pristup!" });
    ...
    async promjeniVidljivost(zahtjev, odgovor) {
    ...
    async obrisiSadrzajIzKolekcije(zahtjev, odgovor) {
