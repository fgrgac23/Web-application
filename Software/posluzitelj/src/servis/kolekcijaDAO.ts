import { KolekcijaDetaljiI, KolekcijaI } from "../servisI/kolekcijaI";
...
export class KolekcijaDAO {
	...
	async dajJavne(): Promise<KolekcijaI[]> {
		let sql = "SELECT * FROM kolekcija WHERE javno = 1";
		return await this.baza.dajPodatke(sql, []);
	}

	async kreirajKolekciju(
		kolekcija: KolekcijaI,
		korisnik_id: number
	): Promise<number> {
		let sql =
			"INSERT INTO kolekcija (naziv, opis, javno, url_slika) VALUES (?,?,?,?)";
		...
		let kolekcijaID: number = podaci.lastID;
	...
	async dodajKorisnikaUKolekciju(
		...
		let sql =
			"INSERT OR REPLACE INTO korisnik_kolekcija (korisnik_id, kolekcija_id, uloga_kolekcija) VALUES (?,?,?)";
	...
	async dajKolekcije(): Promise<KolekcijaDetaljiI[]> {
		...
		let redovi = (await this.baza.dajPodatke(sql, [])) as {
			kolekcija_id: number;
			naziv: string;
			opis: string | null;
			javno: number;
		...
		const rezultat: KolekcijaDetaljiI[] = [];
		let trenutna: KolekcijaDetaljiI | null = null;

		for (const r of redovi) {
			if (trenutna === null || trenutna.kolekcija_id !== r.kolekcija_id) {
				if (trenutna !== null) rezultat.push(trenutna);
	...
	async dajKolekcijeKorisnika(korisnik_id: number) {
	...
	async dajsadrzajZaKolekciju(kolekcija_id: number) {
	...
	async dajjavnisadrzajZaKolekciju(kolekcija_id: number) {
	...
	async promjeniJavnostKolekcije(kolekcija_id: number, javno: number) {
		let sql = "UPDATE kolekcija SET javno = ? WHERE kolekcija_id = ?";
		return await this.baza.ubaciAzurirajPodatke(sql, [javno, kolekcija_id]);
	}

	async postaviIstaknutuSliku(kolekcija_id: number, putanjaSlike: string) {
	...
	async pretraziJavniSadrzaj(pojam: string) {
		let sql = `
		SELECT 
			s.sadrzaj_id,
			s.naslov,
